...
@router.post("/")
async def registrar_egresado_endpoint(request: Request, db: Session = Depends(db_provider.get_db)):
    data = await request.json()
    egresado = egresado_service.registrar_egresado(db, data)
    return {"message": "Egresado registrado correctamente", "id": egresado.id}

@router.get("/")
async def listar_egresados_endpoint(db: Session = Depends(db_provider.get_db)):
    return egresado_service.obtener_egresados(db)

@router.get("/{id}")
async def obtener_egresado_endpoint(id: int, db: Session = Depends(db_provider.get_db)):
...
@router.put("/{id}")
async def editar_egresado_endpoint(id: int, request: Request, db: Session = Depends(db_provider.get_db)):
    data = await request.json()
...
@router.delete("/{id}")
async def eliminar_egresado_endpoint(id: int, db: Session = Depends(db_provider.get_db)):
